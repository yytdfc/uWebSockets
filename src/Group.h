...
protected:
    ...
    std::function<void(WebSocket<isServer> *, char *message, size_t length, OpCode opCode, std::string url)> messageHandler;
...
public:
    ...
    void onMessage(std::function<void(WebSocket<isServer> *, char *, size_t, OpCode, std::string)> handler);
